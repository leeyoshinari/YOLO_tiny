...
class YOLO_tiny(object):
    ...
    def build_network(self, images, keep_prob=0.5, is_training=True):
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            net = slim.conv2d(net, 1024, 3, scope='conv8')
            net = slim.conv2d(net, 1024, 3, scope='conv9')

            net = tf.transpose(net, [0, 3, 1, 2])
            net = slim.flatten(net)
            net = slim.fully_connected(net, 256, scope='local1')
            net = slim.fully_connected(net, 4096, scope='local2')
            net = slim.dropout(net, keep_prob=keep_prob, is_training=is_training, scope='dropout')
            net = slim.fully_connected(net, self.output_size, activation_fn=None, scope='local3')
        return net
